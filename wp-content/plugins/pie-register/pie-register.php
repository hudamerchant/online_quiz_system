	...
	class PieRegister extends PieReg_Base{
		...
		*/
		...
		function piereg_backendregister_scripts(){
			...
			wp_register_style( 'pie_admin_about_us_menu_css', plugins_url("/assets/css/admin-about-us.css",__FILE__),false,'1.0', "all" );
			...
			wp_register_script('pie_admin_about_us_menu_js',plugins_url("/assets/js/admin-about-us.js",__FILE__),'jquery','1.0',false);
		...
		function print_multi_lang_backend_script_vars(){
			...
			foreach($fields_data as $k=>$field){
				...
				if( ($field['type'] == "honeypot") || $field['type']=="submit" || $field['type']=="" || $field['type']=="form" || ($field['type']=="invitation" && $options["enable_invitation_codes"]=="0") ){
					continue;
				}
				...
				if( intval($id) != 0 )
				{
					...
					if($check_form_in_db == false || trim($check_form_in_db) == "")
					{
						...
						if( ($check_form_in_db['Status'] == "enable" && (!isset($check_form_in_db['IsDeleted']) || trim($check_form_in_db['IsDeleted']) != 1) ) || $is_preview){
							if (is_user_logged_in() && !is_admin() && !$is_preview && !$this->is_pr_preview && !$gutenberg_preview ) {
		...
		public function pie_promo_notice_security()
		{
			...
			if ( empty( $show_release_notice ) ) {
                ...
                <div class="notice notice-info pie-admin-notice pie_security_msg is-dismissible" style="border:1px solid #ccd0d4;">
					<h3><?php ( _e( '<img src="'.plugins_url('pie-register').'/assets/images/icon-secure.png" style="height:32px;vertical-align: middle;margin-right:8px;" /> Do You Want to Make Your Registration form More Secure? We hear you.','pie-register')); ?></h3>
...
?>
		...
		function AddPanel()
		{
			...
			$pie_page_suffix_18 = add_submenu_page( 'pie-register', 'About Us', __('About Us',"pie-register") , 'manage_options', 'pie-about-us', array($this, 'PieAboutUs'));
		...
		*/
		...
		function showLoginForm()
		{
			...
			//  For Form with Gutenberg Preview
			$this->is_pr_preview = (isset($_GET['context']) && $_GET['context'] == 'edit') ? true :  $this->is_pr_preview;
